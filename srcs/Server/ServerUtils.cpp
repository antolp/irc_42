...
void Server::broadcastToChannel(const Channel &channel, const std::string &message, int excludeFd)
{
	...
	for (Channel::MemberMap::const_iterator it = members.begin(); it != members.end(); ++it)
	{
		...
		{
			Client *client = findClient(it->first);
			if (client != NULL)
				queueLine(*client, message);
		}
	}
}
...
void Server::sendChannelNames(Client &client, const Channel &channel)
{
	...
	for (Channel::MemberMap::const_iterator it = members.begin(); it != members.end(); ++it)
	{
		Client *memberClient = findClient(it->first);
	...
	sendNumeric(client, "353", "= " + channel.getName() + " :" + namesList);
